...
void loop() {
  ...
  if (estadoBotao1 == LOW) {
    ...
    if (estadoBotao2 == LOW){
      //NowSerial.println(1);
      payload->bitVerificador = true;
    }else{
      //NowSerial.println(0);
      payload->bitVerificador = false;
    }    
    ...
    payload->valorPot = valorPot;

    //Imprime os valores do acelerometro no monitor serial
    payload->acceleration_x = a.acceleration.x;
    payload->acceleration_y = a.acceleration.y;
    payload->acceleration_z = a.acceleration.z;

    payload->gyro_x = g.gyro.x;
    payload->gyro_y = g.gyro.y;
    payload->gyro_z = g.gyro.z;

    NowSerial.write((uint8_t *)payload, sizeof(Payload));
